...
static void mainloop(void)
{
    ...
    clock_gettime(CLOCK_MONOTONIC, &start); //timestamp for start of image capturing
    ...
    clock_gettime(CLOCK_MONOTONIC, &end); //timestamp for after last frame taken
    capturetime = (end.tv_sec - start.tv_sec) + (end.tv_nsec - start.tv_nsec)/	  
    1000000000.0; //finding elapsed time
    printf("AVG FPS is %f\n", frame_count/capturetime);
                // display the time and fps
